...
const allowedExt = new Set(['.pdf', '.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg']);
...
async function findContentRoot() {
  ...
  for (const name of candidates) {
    try {
      ...
      if (stat.isDirectory()) return path.join(root, name);
    } catch {}
  }
  return root;
}
...
async function walk(dir) {
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    if (entry.isDirectory()) {
      ...
      if (allowedExt.has(ext)) items.push({ type: 'file', name: entry.name, path: rel });
    }
  }
...
await fs.writeFile(
  ...
  JSON.stringify({ title: 'Ryan Laird Docs', sourceRoot: path.relative(root, contentRoot).split(path.sep).join('/'), tree }, null, 2),
  'utf8'
);
...
<body>
  ...
  <script>
    ...
    function renderNode(node) {
      if (node.type === 'file') {
        return '<a href="' + encodeURI(node.path) + '" target="_blank" rel="noopener">' + icon('file') + ' ' + node.name + '</a>';
      }
    ...
    fetch('files.json')
      .then(r => r.json())
      .then(data => {
        document.getElementById('meta').textContent = 'Source: ' + data.sourceRoot;
